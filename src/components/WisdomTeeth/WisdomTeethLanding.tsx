import React from 'react'
import Navbar from '../Navbar/Navbar'
import WisdoomTeethHome from './WisdoomTeethHome'
import KerlasLandmark from '../Services/KerlasLandmark'
import WisdomTeethDesc from './WisdomTeethDesc'
import WisdomTeethServices from './WisdomTeethServices'
import Reasons from '../InternalDensity/Reasons'
import VisitJourney from '../InternalDensity/VisitJourney'
import ReadyTransform from '../Services/ReadyTransform'
import Specialists from '../About/Specialists'
import HealthyBenefits from '../InternalDensity/HealthyBenfits'
import GetInTouch from '../Home/GetInTouch'
import RealPeople from '../Services/RealPeople'
import FAQ from '../Home/FAQ'
import Footer from '../Footer/Footer'

const WisdomTeethLanding:React.FC = () => {
  return (
    <div>
       <Navbar/>
       <WisdoomTeethHome/>
       <KerlasLandmark/>
       <WisdomTeethDesc/>
       <WisdomTeethServices/>
       <Reasons/>
       <VisitJourney/>
       <ReadyTransform/>
       <Specialists/>
       <HealthyBenefits/>
       <GetInTouch/>
       <RealPeople/>
       <FAQ/>
       <Footer/>
    </div>
  )
}

export default WisdomTeethLanding
